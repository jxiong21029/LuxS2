...
class UNet(nn.Module):
    def __init__(self, out=20):
        ...
        self.unet = torch.hub.load('mateuszbuda/brain-segmentation-pytorch', 'unet', in_channels=35, out_channels=out, pretrained=False)
...
class LRaspp(nn.Module):
    def __init__(self, out=20):
        ...
        self.lraspp.backbone['0'][0] = nn.Conv2d(35, 16, 3, stride=2, padding=1, bias=False)
        self.types_linear = nn.Linear(out, 13)
        self.resources_linear = nn.Linear(out, 5)
        self.quantities_linear = nn.Linear(out, 1)
