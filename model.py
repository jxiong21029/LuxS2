...
class LuxAIModel(nn.Module):
    ...
    def forward(self, board):
        ...
        )  # shape is now batch_size x 48 x 48 x 1
        predicted_types = self.types_linear(
            predicted_types
        )  # shape is now batch_size x 48 x 48 x 13

        predicted_quantities = (
            self.quantities_conv(board).transpose(1, 3).transpose(1, 2)
        )  # shape is now batch_size x 48 x 48 x 2
        predicted_quantities = self.quantities_linear(
            predicted_quantities
        )  # shape is still batch_size x 48 x 48 x 2
