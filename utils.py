...
class Logger:
    ...
    # log metrics, seen once per epoch
    def log(self, metrics=None, **kwargs):
        metrics = {} if metrics is None else metrics
        for k, v in {**metrics, **kwargs}.items():
            self.cumulative_data[k].append(v)

    # push metrics logged many times per epoch
    # mean and std of these metrics are reported
    ...
    def air_report(self, **kwargs):
        ...
        session.report(
            {k: v[-1] for k, v in self.cumulative_data.items()}, **kwargs
        )
    ...
    def convergence(self, key, p=0.98):
        ...
        A custom metric by me (Jerry), which is close to zero when the metric
        is clearly trending upwards or downwards, and close to one when changes
        in the metric seem to be dominated by noise. Intended for debugging
        purposes, not for scientific reporting.

        p controls the degree to which this metric weights recently measured
        values. p = 0 results in a uniform weighting, independent of time. More
        and more weight is placed on the last few values as p approaches 1.
        """
